...
class DokumenSeeder extends Seeder
{
    public function run()
    {
        ...
        $adminUser = $this->db->table('user')
            ->where('peran', 'admin')
            ->get()
            ->getRow();

        $idPengunggah = $adminUser ? $adminUser->id_user : null;

        // If no admin user exists, skip seeding
        if (!$idPengunggah) {
            echo "Skipping DokumenSeeder: No admin user found in database.\n";
            return;
        }

        // Clear existing dokumen first
        ...
        $uploadDir = FCPATH . 'uploads/dokumen';
        if (!is_dir($uploadDir)) {
